(function(){
  ...
  function seed(){
    ...
    return {
      services:[
        ...
        {id:'pe-mao',name:'Pé e Mão',price:60,duration:120,active:true,description:'Combo completo para mãos e pés no mesmo atendimento.'}
      ],
      businessHours:{
      ...
      appointments:[
        ...
        {id:'a2',customerId:'c2',serviceId:'pe-mao',date:t,start:'14:00',end:'16:00',price:60,status:'agendado',source:'preview'},
  ...
  function normalize(data){
    ...
    let changed=false;
    data.services.forEach(s=>{const expected=s.id==='pe-mao'?120:60;if(Number(s.duration)!==expected){s.duration=expected;changed=true}});
    (data.appointments||[]).forEach(a=>{const svc=data.services.find(s=>s.id===a.serviceId);if(!svc)return;const expectedEnd=minToTime(timeToMin(a.start)+Number(svc.duration));if(a.end!==expectedEnd){a.end=expectedEnd;changed=true}});
  ...
  function slotsFor(data,date,serviceId){
    ...
    const start=timeToMin(h.start), end=timeToMin(h.end), duration=Number(service.duration), out=[];
    const busy=[];
  ...
  function addAppointment(data,{name,phone,serviceId,date,start}){
    ...
    const customer=ensureCustomer(data,name,phone); const end=minToTime(timeToMin(start)+Number(svc.duration));
